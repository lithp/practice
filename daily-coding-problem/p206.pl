...
list_perm_list(Left, Perm, Right) :-
  ...
  */
  ...
  % we're all setup, do the thing!
  check_perm(Perm, 0, Left, Right).
...
check_perm([], _, _, _).
check_perm([FirstIndex|RestIndexes], Position, Left, Right) :-
  nth0(Position, Right, Elem),

  /* This is the call which is affected by the call to is_permutation from earlier.
     this call will immediately fail if it tries to return a value of FirstIndex which
     has already been added to Perm */
  nth0(FirstIndex, Left, Elem),
  NextPosition is Position + 1,
  check_perm(RestIndexes, NextPosition, Left, Right).
